...
namespace algoim
{
    namespace detail
    {
        ...
        booluarray<N,ALGOIM_M> mask_driver(const xarray<T,N>& f, const booluarray<N,ALGOIM_M>& fmask, const xarray<T,N>* g, const booluarray<N,ALGOIM_M>* gmask)
        {
            ...
            auto helper = [&](auto&& self, uvector<int,N> a, uvector<int,N> b)
            {
                ...
                {
                    xa(dim) = T(a(dim)) / ALGOIM_M - eps;
                    xb(dim) = T(b(dim)) / ALGOIM_M + eps;
                }

                if (g)
                {
                    xarray<T,N> fab(nullptr, f.ext()), gab(nullptr, g->ext());
                    algoim_spark_alloc(T, fab, gab);
                ...
                {
                    xarray<T,N> fab(nullptr, f.ext());
                    algoim_spark_alloc(T, fab);
                    bernstein::deCasteljau(f, xa, xb, fab);
        ...
        template<int N, typename T = real>
        booluarray<N,ALGOIM_M> intersectionMask(const xarray<T,N>& f, const booluarray<N,ALGOIM_M>& fmask, const xarray<T,N>& g, const booluarray<N,ALGOIM_M>& gmask)
        {
            return mask_driver<N,T>(f, fmask, &g, &gmask);
        }
        ...
        template<int N, typename T = real>
        booluarray<N,ALGOIM_M> nonzeroMask(const xarray<T,N>& f, const booluarray<N,ALGOIM_M>& fmask)
        {
            return mask_driver<N,T>(f, fmask, nullptr, nullptr);
        }
        ...
        template<int N, typename T = real>
        bool pointWithinMask(const booluarray<N,ALGOIM_M>& mask, const uvector<T,N>& x)
        {
            using std::floor;
            uvector<int,N> cell;
            for (int dim = 0; dim < N; ++dim)
                if constexpr (std::is_same_v<duals::duald, T>)
        ...
        template<int N, typename T = real>
        bool lineIntersectsMask(const booluarray<N,ALGOIM_M>& mask, const uvector<T,N-1>& x, int k)
        {
            using std::floor;
            if constexpr (N > 1)
            {
        ...
        template<int N, typename T = real>
        void restrictToFace(const xarray<T,N>& a, int k, int side, xarray<T,N-1>& out)
        {
        ...
        // ========================================= NOTE =========================================
        template<int N, typename T = real>
        bool resultant_core(const xarray<T,N>& p, const xarray<T,N>* q, int k, xarray<T,N-1>& out)
        {
            assert(0 <= k && k < N);
            ...
            xarray<T,N-1> f(nullptr, out.ext());
            xarray<T,2> mat(nullptr, uvector<int,2>{M, M});
            T *pk, *qk;
            algoim_spark_alloc(T, f, mat);
            algoim_spark_alloc(T, &pk, P, &qk, Q);    

            for (auto i = f.loop(); ~i; ++i)
            {
                uvector<T,N-1> x;
                for (int dim = 0; dim < N - 1; ++dim)
                    x(dim) = T(bernstein::modifiedChebyshevNode(i(dim), f.ext(dim)));
        ...
        template<int N, typename T = real>
        bool resultant(const xarray<T,N>& p, const xarray<T,N>& q, int k, xarray<T,N-1>& out)
        {
        ...
        template<int N, typename T = real>
        bool discriminant(const xarray<T,N>& p, int k, xarray<T,N-1>& out)
        {
            xarray<T,N> prime(nullptr, inc_component(p.ext(), k, -1));
            algoim_spark_alloc(T, prime);
        ...
        template<int N, typename T = real>
        void eliminate_axis(PolySet<N,ALGOIM_M,T>& phi, int k, PolySet<N-1,ALGOIM_M,T>& psi)
        {
            ...
            for (int i = 0; i < phi.count(); ++i)
            {
                ...
                {
                    xarray<T,N-1> p_face(nullptr, remove_component(p.ext(), k));
                    algoim_spark_alloc(T, p_face);
                ...
                xarray<T,N> p_k(nullptr, p.ext());
                algoim_spark_alloc(T, p_k);
                ...
                if (!maskEmpty(disc_mask))
                {
                    ...
                    xarray<T,N-1> disc(nullptr, R);
                    algoim_spark_alloc(T, disc);
            ...
            for (int i = 0; i < phi.count(); ++i) for (int j = i + 1; j < phi.count(); ++j)
            {
                ...
                if (!maskEmpty(mask))
                {
                    ...
                    xarray<T,N-1> res(nullptr, R);
                    algoim_spark_alloc(T, res);
        ...
        uvector<T,N> score_estimate(PolySet<N,ALGOIM_M,T>& phi, uvector<bool,N>& disc)
        {
            ...
            for (int i = 0; i < phi.count(); ++i)
            {
                ...
                {
                    uvector<T,N> x = (j() + 0.5)/T(ALGOIM_M);
    ...
    struct ImplicitPolyQuadrature
    {
        ...
        PolySet<N,ALGOIM_M,T> phi;                                              // Given N-dimensional polynomials
        int k;                                                                  // Elimination axis/height direction; k = N if there are no interfaces
        ImplicitPolyQuadrature<N-1,T> base;                                     // Base polynomials corresponding to removal of axis k
        ...
        std::array<std::tuple<int,ImplicitPolyQuadrature<N-1,T>>,N-1> base_other; // Stores other base cases, besides k, when in aggregate mode
        ...
        ImplicitPolyQuadrature(const xarray<T,N>& p)
        {
        ...
        ImplicitPolyQuadrature(const xarray<T,N>& p, const xarray<T,N>& q)
        {
            {
                auto mask = detail::nonzeroMask(p, booluarray<N,ALGOIM_M>(true));
                if (!detail::maskEmpty(mask))
        ...
        ImplicitPolyQuadrature(const xarray<T,N>& p, const booluarray<N,ALGOIM_M>& pmask, const xarray<T,N>& q, const booluarray<N,ALGOIM_M>& qmask)
        {
            {
        ...
        void build(bool outer, bool auto_apply_TS)
        {
            ...
            else
            {
                ...
                uvector<T,N> score = detail::score_estimate(phi, has_disc);
        ...
        void integrate(QuadStrategy strategy, int q, const F& f)
        {
            ...
            if (k == N)
            {
                ...
                for (MultiLoop<N> i(0, q); ~i; ++i)
                {
                    ...
                    for (int dim = 0; dim < N; ++dim)
                    {
                        x(dim) = T(GaussQuad::x(q, i(dim)));
                        w *= T(GaussQuad::w(q, i(dim)));
                    }
                    f(x, w);
                }
                return;
            ...
            auto integrand = [&](const uvector<T,N-1>& xbase, T w)
            {
                // Allocate node buffer of sufficient size and initialise with {0, 1}
                T *nodes;
                algoim_spark_alloc(T, &nodes, max_count);
                nodes[0] = 0.0;
                nodes[1] = 1.0;
                int count = 2;
                ...
                for (size_t i = 0; i < phi.count(); ++i)
                {
                    ...
                    T *pline, *roots;
                    algoim_spark_alloc(T, &pline, P, &roots, P - 1);
                ...
                for (int i = 0; i < count - 1; ++i)
                {
                    ...
                    if (GL)
                        for (int j = 0; j < q; ++j)
                            if constexpr (std::is_same_v<duals::duald, T>)
                                f(add_component(xbase, k, T(x0 + (x1 - x0) * GaussQuad::x(q, j))), w * (x1 - x0) * T(GaussQuad::w(q, j)));
                            else
                                f(add_component(xbase, k, x0 + (x1 - x0) * GaussQuad::x(q, j)), w * (x1 - x0) * GaussQuad::w(q, j));
                    else
                        for (int j = 0; j < q; ++j)
                            if constexpr (std::is_same_v<duals::duald, T>)
                                f(add_component(xbase, k, T(TanhSinhQuadrature::x(q, j, x0.rpart(), x1.rpart()))), w * TanhSinhQuadrature::w(q, j, x0.rpart(), x1.rpart()));
                            else
                    ...
                    for (int j = 0; j < rcount; ++j)
                    {
                        ...
                        if (detail::pointWithinMask(mask, x))
                        {
                            ...
                            else
                            {
                                ...
                                if (norm(n) > 0)
                                    n *= T(1) / norm(n);
                                T alpha = w * norm(g) / abs(g(k_active));
                                f(x, alpha, alpha * n);
                            }
                        }
                    }
    ...
    template<> struct ImplicitPolyQuadrature<0,duals::duald> {};
} // namespace algoim
